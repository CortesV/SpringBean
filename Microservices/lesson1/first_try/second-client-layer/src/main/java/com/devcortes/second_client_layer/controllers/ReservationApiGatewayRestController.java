package com.devcortes.second_client_layer.controllers;
...
/*	@HystrixCommand(fallbackMethod = "fallbackSingle", groupKey = "Reservation",
	...
	public ResponseEntity<Reservation> getReservation(@PathVariable("reservation_id") Integer id) {
		ResponseEntity<Reservation> exchange = restTemplate.exchange("http://reservation-service/rest/reservations/" + id, HttpMethod.GET, null, new ParameterizedTypeReference<Reservation>() {
		});
		return exchange;
	}
...
/*	@HystrixCommand(fallbackMethod = "fallback", groupKey = "Reservation",
	...
	public ResponseEntity<List<Reservation>> getReservationsWithZuul() {
		ResponseEntity<List<Reservation>> exchange = restTemplate.exchange("http://reservation-service/rest/reservations", HttpMethod.GET, null, new ParameterizedTypeReference<List<Reservation>>() {
		});
		return exchange;
	}
